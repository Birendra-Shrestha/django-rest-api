...
class VoteCreat(generics.CreateAPIView,mixins.DestroyModelMixin):
...
def login(request):
    if request.method=='POST':
        data=JSONParser().parse(request)
        user=authenticate(request, username=data['username'],passowrd=data['password'])

        if user is None:
            return JsonResponse({'error':'Username or password doesnot match'}, status=400)
        else:
            try:
                tokrn=Token.objects.get(user=user)
            except:
                token=Token.objects.create(user=user)
